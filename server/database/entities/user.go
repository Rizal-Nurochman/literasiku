...
type User struct {
	...
	MembershipNumber *string `gorm:"column:membership_number;type:varchar(20);uniqueIndex" json:"membership_number,omitempty"`
...
func (u *User) AfterCreate(tx *gorm.DB) error {
	fmt.Println("AFTER CREATE CALLED")

	number := fmt.Sprintf("LTR%06d", u.ID)
	u.MembershipNumber = &number

	return tx.Model(u).
		Update("membership_number", number).
		Error
}